...
def crawling_news_list(args):
    # url 설정
    search_keyword = f"{(args.search_keyword.replace(' ', '+')).replace('&', '%26')}+{args.dart_class}"
    ...
    while crawling != 0:
        ...
        for i in range(len(area_list)):
            for info in info_list[i].find_all('a'):
                ...
                # TODO: 제목에 단어 들어가 있는지 이 부분 <장래사업계획, 영업잠정실적> 다시!
                keyword_list = [args.search_keyword, args.dart_class]
                if all(word in title_list[i].text for word in keyword_list) and info.get('href').split("//")[1].startswith("news.naver") and info.get('href') not in crawled_news:
